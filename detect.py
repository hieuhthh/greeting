...
def run_demo(net, image_provider, height_size, cpu, track, smooth):
    ...
    for img in image_provider:
        ...
        for pose in current_poses:
            ...
            if Dis(pose.keypoints[4], pose.keypoints[5]) < Thr or Dis(pose.keypoints[7], pose.keypoints[2])  < Thr:
            
                cv2.rectangle(img, (pose.bbox[0], pose.bbox[1]),
                          (pose.bbox[0] + pose.bbox[2], pose.bbox[1] + pose.bbox[3]), (0, 0, 255))
                
                cv2.putText(img, 'Greeting', (pose.bbox[0], pose.bbox[1] - 16),
...
if __name__ == '__main__':
    ...
    fps = 30
    write_video(file_path, video, fps)
